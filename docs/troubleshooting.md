...
## コンパイルでエラーが出て、intelhexがないと言われる
以下のコードを打って、PlatformIOの環境にintelhexを入れましょう。
```
 C:\Users\ユーザー名に合わせる\.platformio\penv\Scripts\pip install intelhex
```
この後にコンパイルし直せばエラーがなくなっているはずです。
...
## うまくいかない時は USB を抜き差しする

ESP32 を接続してもポート一覧に出てこない場合や、書き込み・Serial Monitor がうまくいかない場合があります。
その場合は、一度 USB ケーブルを抜き、数秒待ってから挿し直すと直ることがあります。

- USB ハブを使っている場合は、PC 本体に直接挿して確認する
- 別の USB ポートに挿し直す
- ケーブルを抜き差ししたあと、PlatformIO や Unity のポート一覧を確認し直す
- 認識が不安定な場合は、もう一度 Upload すると直ることがあります
